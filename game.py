...
def snowman(snowman_word):
    ...
    while len(wrong_guesses_list) < SNOWMAN_MAX_WRONG_GUESSES and \
                                    is_word_guessed(snowman_word, 
                                    correct_letter_guess_statuses) == False:
        user_input = get_letter_from_user(correct_letter_guess_statuses,
                                            wrong_guesses_list)

        if user_input in correct_letter_guess_statuses:
            print("You guessed a letter that's in the word!")
            correct_letter_guess_statuses[user_input] = True
            print_word_progress_string(snowman_word,
                                        correct_letter_guess_statuses)
            if is_word_guessed(snowman_word,
                                correct_letter_guess_statuses) == True:
